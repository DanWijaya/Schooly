...
router.post(
  "/lampiran/:id",
  uploadLampiranAnnouncement.array("lampiran_announcement", 10),
  (req, res) => {
    ...
    Announcement.findById(announcement_id, (err, announcement) => {
      ...
      if (!announcement) {
        ...
        if (
          announcement.lampiran != undefined &&
          announcement.lampiran.length > 0
        ) {
    ...
    // res.json({success: "Successfully uploaded the lampiran file", _id: req.params.id})
  }
);
...
router.delete("/lampiran/:id", (req, res) => {
  ...
  for (var i = 0; i < lampiran_to_delete.length; i++) {
    ...
    gfsLampiranAnnouncement.remove(
      { _id: id, root: "lampiran_announcement" },
      (err) => {
        if (err) {
    ...
    if (announcement_id !== "deleteall") {
      ...
      Announcement.findById(announcement_id, (err, ann) => {
        if (!ann) {
          return res
            .status(404)
            .json("Ann object is not found in the Database");
        } else {
