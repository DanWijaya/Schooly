...
router.post("/saveresethash", async (req, res) => {
  ...
  try {
    ...
    let time_in_string = current_time.toLocaleString("id-ID", {
...
router.post("/savepassword", async (req, res) => {
  ...
  try {
    ...
    if (Validator.isEmpty(req.body.password)) {
      result = res.send(
        JSON.stringify({ password_entry: "Kata sandi baru belum diisi" })
      );
    } else if (!Validator.isLength(req.body.password, { min: 8, max: 30 })) {
      result = res.send(
        JSON.stringify({
          password_entry: "Kata sandi harus terdiri dari 8 hingga 30 karakter",
        })
      );
      ...
      result = res.send(
        JSON.stringify({
          reset_problem: "Tautan ini sudah tidak berlaku setelah 5 menit",
          expired: "yes",
        })
      );
    } else {
    ...
    result = res.send(
      JSON.stringify({
        reset_problem:
          "Tautan sudah dipakai sebelumnya, silahkan memohon untuk mengubah kata sandi lagi",
        expired: "yes",
      })
    );
  }
  ...
  User.findOne({ email }).then((user) => {
    ...
    bcrypt.compare(old_password, user.password).then((isMatch) => {
      if (isMatch) {
        ...
        return res
          .status(404)
          .json({ old_password: "Tidak sama dengan kata sandi sekarang" });
      }
    });
  });
});
