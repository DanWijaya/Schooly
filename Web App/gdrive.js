  ...
  return drive.files
    ...
    .then((res) => {
      ...
      if (files.length) {
        ...
        files.forEach((file) => {
          console.log(
            `${file.name} : ${file.parents ? file.parents.length : 0}`
          );
        });
      } else {
...
if (type === "t") {
  ...
  const pushFiles = (auth, id, iterator, ms) => {
    ...
    return clientUploadFile(auth).catch((err) => {
      ...
      if (err.code === 403) {
        console.log(
          `Upload id: ${id}, error:  ${err.response.data.error.message}, retry number: ${iterator}`
        );
  ...
  Promise.all(uploads).then(() => {
    t1 = performance.now();
    console.log(
      "Semua upload sudah selesai! Waktu yang diperlukan: " +
        (t1 - t0) +
        " milliseconds."
    );
  });
