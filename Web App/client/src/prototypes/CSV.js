...
export default function CSV() {
	...
	const showTable = () => {
		let element = (<Typography variant="h6" style={{padding: '60px 0', margin: 'auto'}}>File CSV belum dipilih</Typography>);
	...
	return (
		<div className={classes.root}>
          ...
          <form onSubmit={(event) => {handleSubmit(event)}} className={classes.form} style={{display: 'flex', justifyContent: 'center'}}>
            ...
            <button type="submit" ref={submitButton} style={{display:'none'}}>Submit</button>
				...
				<Grid container item xs={12} sm={4}>
					<Grid item xs={12}  className={classes.gridButton}>
						<Button variant="contained" onClick={() => {handleClickSubmit()}} className={classes.button}>Submit</Button>
					</Grid>
					<Grid item xs={12} className={classes.gridButton}>
			...
			<Dialog fullWidth={true} maxWidth="sm" onClose={() => {handleCloseDialog()}} open={openTabelDialog}>
				{showTable()}
			</Dialog>

			<Snackbar open={openSuccessAlert} autoHideDuration={2000} onClose={(event, reason) => {handleCloseSuccessAlert(event, reason)}}>
				<MuiAlert elevation={6} variant="filled" onClose={(event, reason) => {handleCloseSuccessAlert(event, reason)}} severity="success">
					Isi CSV siap ditampilkan!
				</MuiAlert>
			</Snackbar>

			<Snackbar open={openErrorAlert} autoHideDuration={2000} onClose={(event, reason) => {handleCloseErrorAlert(event, reason)}}>
				<MuiAlert elevation={6} variant="filled" onClose={(event, reason) => {handleCloseErrorAlert(event, reason)}} severity="error">
