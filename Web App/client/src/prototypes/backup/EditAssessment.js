...
class EditAssessment extends Component {
  ...
  formatQstNumber = (numberIndexArray) => {
    ...
    if (numberIndexArray.length === 1) {
      ...
      qstNumbers +=
        numberIndexArray[0] + 1 + " dan " + (numberIndexArray[1] + 1);
    } else {
      ...
      updateAssessment(
        ...
        .catch((err) =>
          this.handleOpenErrorSnackbar(
            `Keterangan ${this.state.type} masih kosong!`
          )
        );
    } else {
      this.handleOpenInvalidQstSnackbar(
        `Soal nomor ${this.formatQstNumber(
  ...
  render() {
    ...
    const linkToShare = `https://${window.location.host}/kuis-murid/${this.props.match.params.id}`;
                ...
                <Paper>
                  ...
                  >
                    ...
                    >
                      ...
                      <Grid item>
                        <Tooltip
                          title={`Salin ID ${this.state.type} ke clipboard`}
                        >
                          <IconButton
                            onClick={(e) => {
