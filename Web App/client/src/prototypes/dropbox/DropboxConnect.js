  ...
  return (
    ...
    >
      <div onClick={() => handleUpdatePath("")} style={{ display: "flex" }}>
        <HomeIcon
          color={path ? "textPrimary" : "primary"}
          className={classes.homeDirectoryIcon}
        />
        <Typography
          color={path ? "textPrimary" : "primary"}
          className={classes.viewDirectoryPath}
        >
          Dropbox
        </Typography>
      </div>
        ...
        if (i !== 0) {
          return (
            <Grid item onClick={() => handleUpdatePath(dir_list[i - 1])}>
              <Typography
                color={
                  i === folders_name.length - 1 ? "primary" : "textPrimary"
                }
                className={classes.viewDirectoryPath}
              >
                {name}
...
function DropboxConnect(props) {
  ...
  const getLinkToFile = useCallback(
    (path) => {
      // console.log(path);
      let dropbox = new Dropbox({ fetch: fetch, accessToken: dropbox_token });
      ...
      <div className={classes.root}>
        ...
        >
          <Alert
            ...
            icon={<CircularProgress disableShrink size={22} color="inherit" />}
          >
            {loadingMessage}
          </Alert>
        </Snackbar>
        ...
        <Grid
          ...
          alignItems="center"
          style={{ marginTop: "20px", marginBottom: "7.5px" }}
        >
        ...
        <Grid container direction="column" spacing={5} alignItems="center">
          <Grid item container spacing={3} alignItems="center">
            ...
            <Grid item>
              ...
              <Typography color="error">Belum Terhubung</Typography>
            </Grid>
          </Grid>
          <Grid
            item
...
export default connect(mapStateToProps, { setDropboxToken })(DropboxConnect);
