...
function ViewSubject(props) {
  ...
  function listMaterials(category=null, subject={}, tab="pekerjaan_kelas"){
    ...
    if(Boolean(selectedMaterials.length)) {
      ...
      for (var i = selectedMaterials.length-1; i >= 0; i--){
        ...
        if(!category || (category === "subject" && material.subject === subject)){
          materialList.push(
            <MaterialListitem
  ...
  function listTasks(category=null, subject=null, tab="pekerjaan_kelas"){
    ...
    if (Boolean(tasksCollection.length)) {
      ...
      for (i = 0; i < tasksList.length; i++){
        ...
        // }
        if(tab === "pekerjaan_kelas"){
          if((!category || (category === "subject" && task.subject === subject)) && workStatus === "Belum Dikumpulkan"){
            result.push(
              <AssignmentListItem
                work_title={task.name}
                work_category_avatar={workCategoryAvatar}
          ...
          if(!category || (category === "subject" && task.subject === subject)){
            result.push(
              <AssignmentListItem
                work_title={task.name}
                work_category_avatar={workCategoryAvatar}
  ...
  function listAssessments(category=null, subject=null, type, tab="pekerjaan_kelas"){
    let AssessmentsList = []
    let result = [];
...
export default connect(
  mapStateToProps, {setCurrentClass,
    getAllSubjects, getAllTask, getAllTaskFilesByUser, getMaterial, getAllAssessments}
) (ViewSubject)
