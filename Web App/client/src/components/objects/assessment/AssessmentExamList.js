  ...
  return (
    <div className={classes.root}>
      ...
      <DeleteDialog
        ...
        deleteItem={() => {
          onDeleteAssessment(selectedAssessmentId, "Ujian");
        }}
        isWarning={true}
      />
      <Dialog
      ...
      <Grid container direction="column" spacing={2}>
        {rows.length === 0 ? (
          ...
          stableSort(rows, getComparator(order, orderBy)).map((row, index) => {
            ...
            let linkToShare = `https://schoolysystem.com/kuis-murid/613e0fdb095d7a0eeb7d1777://${window.location.host}/ujian-murid/${row._id}`;
            return (
              <Grid item>
                {user.role === "Teacher" ? (
